import {
  PrismaClient,
  Match,
  PgrDataSource,
  PgrValidationStatus,
  Prisma,
} from "@prisma/client";
import type { SetScore } from "@ping-pang/pgr-core";

export interface CreateMatchData {
  playerAId: string;
  playerBId: string;
  winnerId?: string;
  scoreA?: number;
  scoreB?: number;
  setsDetail?: SetScore[];
  playedAt: Date;
  competitionId?: string;
  source: PgrDataSource;
  importId?: string;
}

export interface GetMatchesOptions {
  from?: Date;
  to?: Date;
  validationStatus?: PgrValidationStatus;
  source?: PgrDataSource;
  limit?: number;
}

export class MatchesService {
  constructor(private readonly prisma: PrismaClient) {}

  async create(data: CreateMatchData): Promise<Match> {
    return this.prisma.match.create({
      data: {
        player_a_id: data.playerAId,
        player_b_id: data.playerBId,
        winner_id: data.winnerId,
        score_a: data.scoreA,
        score_b: data.scoreB,
        sets_detail: data.setsDetail
          ? (data.setsDetail as unknown as Prisma.InputJsonValue)
          : undefined,
        played_at: data.playedAt,
        competition_id: data.competitionId,
        source: data.source,
        import_id: data.importId,
        validation_status: "PENDING",
      },
    });
  }

  async confirm(matchId: string): Promise<Match> {
    return this.prisma.match.update({
      where: { id: matchId },
      data: { validation_status: "CONFIRMED" },
    });
  }

  async dispute(matchId: string): Promise<Match> {
    return this.prisma.match.update({
      where: { id: matchId },
      data: { validation_status: "DISPUTED" },
    });
  }

  async reject(matchId: string): Promise<Match> {
    return this.prisma.match.update({
      where: { id: matchId },
      data: { validation_status: "REJECTED" },
    });
  }

  async getPlayerMatches(
    playerId: string,
    options: GetMatchesOptions = {}
  ): Promise<Match[]> {
    const where: Prisma.MatchWhereInput = {
      OR: [{ player_a_id: playerId }, { player_b_id: playerId }],
      ...(options.from || options.to
        ? {
            played_at: {
              ...(options.from ? { gte: options.from } : {}),
              ...(options.to ? { lte: options.to } : {}),
            },
          }
        : {}),
      ...(options.validationStatus
        ? { validation_status: options.validationStatus }
        : {}),
      ...(options.source ? { source: options.source } : {}),
    };

    return this.prisma.match.findMany({
      where,
      orderBy: { played_at: "desc" },
      take: options.limit,
    });
  }

  /**
   * Get all CONFIRMED matches in a date range — used for rating period processing.
   */
  async getConfirmedMatchesInPeriod(from: Date, to: Date): Promise<Match[]> {
    return this.prisma.match.findMany({
      where: {
        validation_status: "CONFIRMED",
        played_at: { gte: from, lte: to },
      },
      orderBy: { played_at: "asc" },
    });
  }

  async findById(id: string): Promise<Match | null> {
    return this.prisma.match.findUnique({ where: { id } });
  }
}
